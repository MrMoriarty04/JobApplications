...
def GUINew(self):    
    ...
    cols = ("_Person__fname", "_Person__mname", "_Person__lname", "_Person__birthDate", "_Person__id","_Person__gpa", "_Person__email", "_Person__gender","_Person__specialization","_Person__phone")
    treeview = ttk.Treeview(treeFrame, show="headings",
                            yscrollcommand=treeScroll.set, columns=cols, height=50)
...
def data_show():
    ...
    def validateName(name):
            name = re.sub(r"(.)\1\1+", r"\1", name.lower())
    ...
    class Person:
        ...
        def setId(self, id):
            self.__id = id if re.search(r"^(9[2-9][0-9][01]|2000)\d{6}$", id) else None
        ...
        def setSpecialization(self, specialization):
            specializationPattern = r"^(cs|cis|se|ai|ds|it|computer\ss(cience)?|computer\sinformation\ssystem|software\sengineer|artificial\sscience|cyber\ssecurity|data\sscience|information\stechnology)$"
            if re.search(specializationPattern, specialization.lower()):
        ...
        # Graduated
        def getGraduated(self):
            return self.__graduated

        def setGraduated(self, graduated):
            self.__graduated = True if re.search(r"^([Gg][Rr]?[Aa]?[Dd]?[Uu]?[Aa]?[Tt]?[Ee]?[Dd]?)|1|[Tt]r?u?e?|[Yy][Ee]?[Ss]?$" , graduated.lower()) else False
